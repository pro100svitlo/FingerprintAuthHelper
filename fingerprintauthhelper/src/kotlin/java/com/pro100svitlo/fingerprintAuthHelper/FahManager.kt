...
internal class FahManager() : FingerprintManager.AuthenticationCallback() {
    ...
    class Builder(internal val mContext: Context, internal val l: FahListener) {
    ...
    internal fun canListen(showError: Boolean): Boolean {
        ...
        if (!isHardwareEnabled()) {
            ...
            logThis("canListen failed. reason: " + mContext!!.getString(R.string.HARDWARE_DISABLED))
            return false
        }
        ...
        if (!keyguardManager!!.isKeyguardSecure) {
            ...
            logThis("canListen failed. reason: " + mContext!!.getString(R.string.LOCK_SCREEN_DISABLED))
            return false
        }
    ...
    private fun isFingerprintEnrolled(showError: Boolean): Boolean {
        if (!mFingerprintManager!!.hasEnrolledFingerprints()) {
            ...
            logThis("canListen failed. reason: " + mContext!!.getString(R.string.NO_FINGERPRINTS))
            mSecureElementsReady = false
            return false
        }
        return true
    ...
    private fun registerBroadcast(register: Boolean) {
        if (mTimeOutLeft > 0 && register && !mBroadcastRegistered) {
            ...
            mContext!!.registerReceiver(timeOutBroadcast, IntentFilter(FahConstants.TimeOutService.TIME_OUT_BROADCAST))
    ...
    private fun runTimeOutService() {
        ...
        if (mTimeOutIntent == null) {
            mTimeOutIntent = Intent(mContext, FahTimeOutService::class.java)
        }
